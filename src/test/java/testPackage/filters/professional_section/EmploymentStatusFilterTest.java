...
public class EmploymentStatusFilterTest extends basePage {
    ...
    private static class EmploymentStatusRow {
        ...
        EmploymentStatusRow(String candidateName, String filterValue, String valueOnDetails, boolean match) {
    ...
    public void setUpSshTunnel() {
        System.out.println("\n[FRAMEWORK] BeforeClass (EmploymentStatusFilter): Starting SSH tunnel");
        SSHTunnelManager.startTunnel();
    }

    @AfterClass(alwaysRun = true)
    public void tearDownSshTunnel() {
        System.out.println("\n[FRAMEWORK] AfterClass (EmploymentStatusFilter): Stopping SSH tunnel");
    ...
    @Test
    @Description("Professional Details – Employment Status filter from Excel; sample candidates and assert Employment Status on details page")
    public void employmentStatusFilter_FromExcel_SampleAndAssert() throws Exception {
        ...
        if (Files.exists(Paths.get(excelPath))) {
            try (InputStream in = Files.newInputStream(Paths.get(excelPath))) {
                statuses = employment_status_filter_Util.readStatuses(in);
            }
        }
        if (statuses.isEmpty()) {
        ...
        for (String filterValue : statuses) {
            ...
            if (totalCount <= 0) {
                System.out.println("[EMPLOYMENT STATUS] No results for '" + filterValue + "'. Skipping.");
                continue;
            }
            ...
            System.out.println("\n" + "=".repeat(72));
            System.out.println("EMPLOYMENT STATUS FILTER: " + filterValue + " | Total results: " + totalCount + " | Sampled: " + rows.size());
